<template>
  <div>
    <div class="uk-card-header">
      <div class="uk-width-expand">
        <h3
          class="uk-card-title uk-margin-remove-bottom uk-text-bold uk-text-center"
        >
          {{ trans.get("pendingCashoutTransaction.title") }}
        </h3>
      </div>
    </div>
    <div class="uk-card-body uk-padding-small">
      <ul uk-accordion="multiple: true">
        <li
          v-for="(transaction, index) in transactions"
          :id="`csh_trnx_${transaction.id}`"
          :key="`csh_trnx_${index}`"
          :class="['uk-open', 'uk-margin-remove-top',
            transaction.type == 'investment' ? 'blue' : 'green',
            'accent-1'
          ]"
        >
          <template>
            <a class="uk-accordion-title uk-padding-small white-text" href="#">
              <i class="mdi mdi-arrow-down green-text"></i>
              <div
                class="uk-grid-small uk-child-width-auto uk-flex-inline"
                uk-grid
                :uk-countdown="`date: ${transaction.count_down}`"
              >
                <div class="uk-text-small">
                  <div
                    class="uk-countdown-days uk-text-center uk-text-bold"
                  ></div>
                  <div
                    class="uk-countdown-label uk-margin-remove-top uk-margin-small uk-text-center uk-visible@s"
                  >
                    {{ trans.get("pendingCashoutTransaction.days") }}
                  </div>
                </div>
                <div
                  class="uk-countdown-separator uk-text-small"
                  style="padding-left:5px;"
                >
                  :
                </div>
                <div class="uk-text-small" style="padding-left:5px;">
                  <div
                    class="uk-countdown-hours uk-text-center uk-text-bold"
                  ></div>
                  <div
                    class="uk-countdown-label uk-margin-remove-top uk-margin-small uk-text-center uk-visible@s"
                  >
                    {{ trans.get("pendingCashoutTransaction.hours") }}
                  </div>
                </div>
                <div
                  class="uk-countdown-separator uk-text-small"
                  style="padding-left:5px;"
                >
                  :
                </div>
                <div class="uk-text-small" style="padding-left:5px;">
                  <div
                    class="uk-countdown-minutes uk-text-center uk-text-bold"
                  ></div>
                  <div
                    class="uk-countdown-label uk-margin-remove-top uk-margin-small uk-text-center uk-visible@s"
                  >
                    {{ trans.get("pendingCashoutTransaction.minutes") }}
                  </div>
                </div>
                <div
                  class="uk-countdown-separator uk-text-small"
                  style="padding-left:5px;"
                >
                  :
                </div>
                <div class="uk-text-small" style="padding-left:5px;">
                  <div
                    class="uk-countdown-seconds uk-text-center uk-text-bold"
                  ></div>
                  <div
                    class="uk-countdown-label uk-margin-remove-top uk-margin-small uk-text-center uk-visible@s"
                  >
                    {{ trans.get("pendingCashoutTransaction.seconds") }}
                  </div>
                </div>
              </div>
              <span class="uk-label uk-label-danger uk-float-right">{{
                Number(transaction.amount)
              }}</span>
            </a>
            <div
              class="uk-accordion-content uk-padding-small uk-padding-remove-top"
            >
              <div class="uk-text-center">
                <div
                  class="uk-button-group uk-flex uk-flex-center uk-flex-middle"
                >
                  <button
                    v-if="transaction.status == 'pending'"
                    title="Confirm Transaction"
                    class="uk-button uk-button-medium white lighten-3 blue-text"
                    @click="confirm_transaction(transaction.id)"
                  >
                    <span class="mdi mdi-18px mdi-check"></span> {{ trans.get("pendingCashoutTransaction.confirm") }}
                  </button>
                  <button
                    title="View Sender Details"
                    class="uk-button uk-button-medium white lighten-3 blue-text"
                    @click="view_sender(transaction.id)"
                  >
                    <span class="mdi mdi-18px mdi-account"></span> {{ trans.get("pendingCashoutTransaction.sender") }}
                  </button>
                  <button
                    v-if="transaction.extended_by == (null || 0)"
                    title="Extend Sender Time"
                    class="uk-button uk-button-medium white lighten-3 blue-text"
                    @click="extend_sender_time(transaction.id)"
                  >
                    <span class="mdi mdi-18px mdi-clock"></span> {{ trans.get("pendingCashoutTransaction.time") }}
                  </button>
                  <button
                    v-if="transaction.pop != null"
                    title="View Proof of payment"
                    class="uk-button uk-button-medium white lighten-3 blue-text"
                    @click="view_pop(transaction.pop)"
                    :id="`vpop_${transaction.type}_${transaction.id}`"
                  >
                    <span class="mdi mdi-18px mdi-image"></span>POP
                  </button>
                </div>
              </div>
            </div>
          </template>
        </li>
      </ul>
    </div>
    <div class="uk-text-center uk-card-footer">
      <paginate
        v-if="pagination_data.record_count > 0"
        v-model="pagination_data.current_page"
        :page-count="pagination_data.page_count"
        :page-range="3"
        :margin-pages="2"
        :prev-text="'<span uk-pagination-previous></span>'"
        :next-text="'<span uk-pagination-next></span>'"
        :container-class="'uk-pagination uk-flex-center'"
        :active-class="'uk-active'"
        :disable-class="'uk-disabled'"
        :click-handler="page_swap"
      >
      </paginate>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      pagination_data: {
        page_count: 0,
        current_page: 1,
        record_count: 0
      },
      transactions: []
    };
  },
  methods: {
    load_data(page = null) {
      let api_url = "";
      if (page == null) {
        api_url = `${window.location.origin}/api/user/transaction/cashout_list/${Laravel.userId}`;
      } else {
        api_url = page;
      }
      axios
        .get(api_url)
        .then(res => {
          this.load_transactions_data(res.data.transactions.data);
          this.load_pagination_data(
            res.data.transactions.last_page,
            res.data.transactions.current_page,
            res.data.transactions.total
          );
          return;
        })
        .catch(err => {
          console.log({ err: err.message });
        });
    },
    load_transactions_data(data) {
      this.transactions = data;
      return;
    },
    load_pagination_data(last_page, current_page, total_records) {
      this.pagination_data = {
        page_count: last_page,
        current_page: current_page,
        record_count: total_records
      };
      return;
    },
    page_swap(page = this.pagination_data.current_page) {
      if (page > this.pagination_data.page_count || page < 1) {
        let page = 1;
      }
      this.load_data(
        `${window.location.origin}/api/user/transaction/cashout_list/${Laravel.userId}?page=${page}`
      );
      return;
    },
    submit_pop(transaction_id) {
      let selected_transaction = transaction_id;
      this.$swal.fire({
        showCloseButton: true,
        showCancelButton: true,
        confirmButtonText: "Select",
        cancelButtonText: "Cancel",
        title: "Select POP",
        html:
          "<img data-src='data:image/png;base64,' width='400' height='200' alt='pop placeholder' style='height:40vh;object-fit:cover;' class='uk-width-1-1 uk-border-rounded' uk-img>",
        input: "file",
        inputAttributes: {
          accept: "image/*",
          "aria-label": "Upload your proof of payment picture"
        },
        preConfirm: file => {
          let file_obj = file;
          this.$swal.fire({
            showCloseButton: true,
            showCancelButton: true,
            confirmButtonText: "Upload",
            cancelButtonText: "Cancel",
            title: "Upload POP",
            html: `<img data-src='${URL.createObjectURL(
              file
            )}' width='400' height='200' alt='selected pop image' style='height:40vh;object-fit:cover;' class='uk-width-1-1 uk-border-rounded' uk-img>`,
            showLoaderOnConfirm: true,
            preConfirm: () => {
              let pop_form = new FormData();
              pop_form.append("user_id", Laravel.userId);
              pop_form.append("pop", file_obj);
              pop_form.append("transaction_id", selected_transaction);
              const pop_submit_url = `${window.location.origin}/api/user/transaction/pop/upload`;
              const axios_config = {
                headers: {
                  "Content-Type": "multipart/form-data",
                  Accept: "application/json"
                }
              };
              axios
                .post(pop_submit_url, pop_form, axios_config)
                .then(res => {
                  this.$swal.fire({
                    title: "Successful" + `: ${res.statusText}`,
                    icon: "success",
                    text: `${res.data.message}`
                  });
                })
                .catch(err => {
                  this.$swal.fire({
                    title: "Failed: " + `${err.response.statusText}`,
                    icon: "error",
                    text: `${err.response.data.message}`
                  });
                })
                .finally(() => {
                  this.load_data();
                });
            },
            allowOutsideClick: () => !this.$swal.isLoading()
          });
        }
      });
    },
    extend_sender_time(transaction_id) {
      let selected_transaction = transaction_id;
      this.$swal.fire({
        showCloseButton: true,
        showCancelButton: true,
        confirmButtonText: "Select",
        cancelButtonText: "Cancel",
        title: "Select Time to Extend",
        input: "select",
        inputOptions: [...Array(24).keys()].map(i => i + 1),
        inputPlaceholder: "Select Hour",
        preConfirm: selected_time => {
          let extensible_time = Number(selected_time) + 1;
          this.$swal.fire({
            showCloseButton: true,
            showCancelButton: true,
            confirmButtonText: "Extend",
            cancelButtonText: "Cancel",
            title: "Extend Time",
            text: `Extend Time of transaction by ${extensible_time} Hour(s)`,
            showLoaderOnConfirm: true,
            preConfirm: () => {
              const axios_config = {
                headers: {
                  "Content-Type": "multipart/form-data",
                  Accept: "application/json"
                }
              };
              axios
                .get(
                  `${window.location.origin}/api/user/transaction/extend/${selected_transaction}/time/${extensible_time}`,
                  axios_config
                )
                .then(res => {
                  this.$swal.fire({
                    title: "Successful" + `: ${res.statusText}`,
                    icon: "success",
                    text: `${res.data.message}`
                  });
                })
                .catch(err => {
                  this.$swal.fire({
                    title: "Failed: " + `${err.response.statusText}`,
                    icon: "error",
                    text: `${err.response.data.message}`
                  });
                })
                .finally(() => {
                  this.load_data();
                });
            },
            allowOutsideClick: () => !this.$swal.isLoading()
          });
        },
        allowOutsideClick: () => !this.$swal.isLoading()
      });
    },
    view_pop(param) {
      let pop = param;
      this.$swal.fire({
        showCloseButton: true,
        confirmButtonText: "Ok",
        title: "Viewing POP",
        html: `<img data-src='${window.location.origin}/image/pop/${pop}' width='400' height='200' alt='pop placeholder' style='height:50vh;object-fit:cover;' class='uk-width-1-1 uk-border-rounded' uk-img>`,
        onDestroy: () => {
          pop = null;
        }
      });
    },
    view_sender(transaction_id) {
      axios
        .get(
          `${window.location.origin}/api/user/transaction/find/${transaction_id}/sender`
        )
        .then(res => {
          let sender_details = "";
          Object.keys(res.data.sender).map(function(key, index) {
            sender_details += `<li class="uk-padding-remove">
            <div class="uk-grid-small uk-margin-small-top uk-text-center uk-grid-match" uk-grid>
              <div class='uk-width-2-5'>
                <div class="uk-grid-small uk-flex-middle uk-text-left" uk-grid>
                  <div class="uk-width-expand">
                    <h5 class="uk-text-bold">${key}: </h5>
                  </div>
                </div>
              </div>
              <div class='uk-width-3-5'>
                <div class="uk-grid-small uk-flex-middle uk-text-left" uk-grid>
                  <div class="uk-width-expand">
                    <h5>${res.data.sender[key]}</h5>
                  </div>
                </div>
              </div>
            </div>
          </li>`;
          });
          this.$swal.fire({
            showCloseButton: true,
            confirmButtonText: "Ok",
            title: "Sender Details",
            html: `<ul id='sender_details' class='uk-list uk-list-divider'>${sender_details}</ul>`
          });
        })
        .catch(err => {
          console.log({ err: err.message });
        });
    },
    confirm_transaction(transaction_id) {
      this.$swal
        .fire({
          title: "Confirm?",
          text: "You won't be able to revert this!",
          icon: "warning",
          showCancelButton: true,
          confirmButtonText: "Confirm",
          cancelButtonText: "Cancel",
          allowOutsideClick: () => !this.$swal.isLoading()
        })
        .then(result => {
          if (result.value) {
            axios
              .get(
                `${window.location.origin}/api/user/transaction/confirm/${transaction_id}`
              )
              .then(res => {
                console.log({ confirmed: res.data });
                this.$swal.fire({
                  title: `Success: ${res.statusText}`,
                  text: `${res.data.message}`,
                  icon: "success",
                  onDestroy: () => {
                    this.load_data();
                  }
                });
                return;
              })
              .catch(err => {
                console.log({ err: err.message });
                this.$swal.fire({
                  title: "Failed: " + `${err.response.statusText}`,
                  icon: "error",
                  text: `${err.response.data.message}`,
                  onDestroy: () => {
                    this.load_data();
                  }
                });
              });
          }
        });
    }
  },
  created() {
    this.load_data();
  }
};
</script>
